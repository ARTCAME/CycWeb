...
function gestionPopUps(el) {
	...
	/*if (!esTactil) {
		if (elementoPopUpper) {
	       	k.scrollX = window.scrollX;
	    	k.scrollY = window.scrollY;
	...
	if (elementoPopUpper) {
    	k.scrollX = window.scrollX; /* Guardamos para luego volver a este punto */
		k.scrollY = window.scrollY; /* Guardamos para luego volver a este punto */
    	document.documentElement.style.position = "fixed";
		document.body.style.position = "fixed";
    	document.body.style.top = "-" + k.scrollY + "px";
	}
...
function reseteaScroll() {
	...
	window.scrollTo(0, k.scrollY); /* Restablecemos el scroll */	
...
function cerrarEsc(ev) {
    ...
    if (tecla == 27) { /* Si se ha pulsado la tecla escape... */
    	...
    	if (document.getElementById("contenedor-ampli").className == "trans-galeria") {
...
function actualDiv(n) { 
	...
	muestraDiv(indiceImagen = n); 
}
function sumaDiv(n) { muestraDiv(indiceImagen += n); }
var cerrar;
function muestraDiv(n) {
	...
	if (n > x.length) { indiceImagen = 1; } /* Se vuelve al principio si estamos en el final */
	if (n < 1) { indiceImagen = x.length; } /* Si estamos al principio y se quiere ir al final */
	var img = document.getElementById("miImg"+indiceImagen);  /* Guardamos imagen apuntada por indiceImagen */
	...
	caption.innerHTML = x[indiceImagen-1].alt; /* Muestra el texto descriptivo de cada imagen */
	cual.innerHTML = indiceImagen + "/" + x.length; /* Muestra la imagen actual */
}
...
function verEscapeIframe(e) {
	...
	/*cerrar = document.getElementById("terminos-pie").contentWindow.document.getElementById("btn-cerrar-term")
	cerrar.onclick = function() { cerrarAspa(); }*/
}
