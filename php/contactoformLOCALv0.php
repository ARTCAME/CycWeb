<?php
	...
	if ($response != null && $response->success) {
		...
		$errorConexion = "<div class='resultado'>No se ha podido crear una conexión con la base de datos.<br>Puedes volver a intentarlo y si el problema persiste contacta con nosotros por cualquiera de las <a href='#formas-contacto'>otras vías disponibles</a>.<br>Incidencia registrada automáticamente<span class='cerrar' onclick='cerrarMensaje()' title='Cerrar'>&times;</span></div>";
		// Comprobación de conexión de usuario de guardado de registro, si este funciona comprobamos los demás
		$userReg = "loggercyc";
		$passReg = "charcharcucu";
		...
		if ($conReg->connect_errno) { // Si la conexión da error generamos registro en archivo de texto y envío de e-mail
			...
			$userSelect = "consultercyc";
			$passSelect = "carchuchucar";
			...
			$userInsert = "insertercyc";
			$passInsert = "charcucharcu";
		...
		// Si se obtiene por post el botón quiere decir que se ha enviado el formulario y se leen el resto de elementos por post, las conexiones $conReg, $conInsert y $conSelect deberán ser correctas para poder registrar en la base de datos la consulta
			...
			if ($cont > 5) { // Si se ha utilizado el correo electrónico más de 5 veces el mismo día se muestra error
				...
				// Prevención de inyección SQL
				$stmt = $conInsert->prepare("INSERT INTO `cyc`.`consultas` (`id_consulta`,`nombre`,`apellidos`,`telefono`,`email`,`consulta`,`fecha_creacion`,`publicidad`) VALUES (NULL,?,?,?,?,?,DEFAULT,?);");
				if ($conInsert->error) { // Si la consulta produce error
					$result = "<div class='resultado'>El contacto no se pudo enviar.<br>Puedes volver a intentarlo y si el problema persiste contacta con nosotros por cualquiera de las <a href='#contacto-dire'>otras vías disponibles</a>.<br>Incencia registrada automáticamente<span class='cerrar' onclick='cerrarMensaje()' title='Cerrar'>&times;</span></div>";
					...
					$result = "<div class='resultado-correcto'>Contacto enviado correctamente.<br>En breve gestionaremos tu consulta.<br>¡Muchas gracias!<span class='cerrar' onclick='cerrarMensaje()' title='Cerrar'>&times;</span></div>";
	...
	function creacionRegistroEnBD($conReg,$enviarError,$codigoError,$result,$ultimoId) {
		...
		// Prevención de inyección SQL
		$stmt = $conReg->prepare("INSERT INTO `cyc`.`registro` (`id_registro`,`fecha_creacion`,`IP`,`codigoerror`,`descripcionerror`,`resultado`,`nombre`,`telefono`,`email`,`consulta`) VALUES (NULL,DEFAULT,?,?,?,?,?,?,?,?);");
		...
		if ($codigoError == NULL && $ultimoId != NULL) { // Si la variable $codigoError está vacía y se ha pasado valor en $ultimoId se modifica la última inserción para añadir la FK que nos da $ultimoId
			// Prevención de inyección SQL
			$stmt = $conReg->prepare("UPDATE `cyc`.`registro` SET id_consulta=? WHERE id_registro=?;");
			if ($conReg->error) { // Si la consulta produce error
				...
				enviarEmail($conReg->connect_error,$conReg->connect_errno,$result,$detalle); // Envío de e-mail confirmando todo ok
			}
		}	
	}	
	...
	function enviarEmail($enviarError,$codigoError,&$result,$detalle) {
		...
		$mail->Username = "contacto.charcuteriacyc@gmail.com";         // SMTP username
		$mail->Password = "1234567812345678";   // SMTP password
		...
		$mail->setLanguage("es", "PHPMailer/");	// Error language
		$mail->setFrom("contacto.charcuteriacyc@gmail.com");
		$mail->addAddress("contacto.charcuteriacyc@gmail.com"); // Add a recipient
